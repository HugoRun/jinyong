...
<p>
	...
	[图片] 
	<br/>
	<%  AccouteVO accouteVO = (AccouteVO) zhuangBeiDAO.getAccouteView(zbid);
	    ArmVO armVO = (ArmVO) zhuangBeiDAO.getArmView(zbid);
	    JewelryVO jewelryVO = (JewelryVO) zhuangBeiDAO.getJewelry(zbid);
	...
	卖出价格:
     <%if (Integer.parseInt(fl) == 1) {  %>
	 <%=accouteVO.getAccSell() %>铜钱
	 <%} if (Integer.parseInt(fl) == 2) { %>
	 <%=armVO.getArmSell() %>铜钱
	 <%} if (Integer.parseInt(fl) == 3) { %>
	 <%=jewelryVO.getJewSell() %>铜钱
	 <%} %>  
	<br/>
	使用等级:
     <%if (Integer.parseInt(fl) == 1) {  %>
	...
	性别要求:
    <%if (Integer.parseInt(fl) == 1) {  %>
	<%if(accouteVO.getAccSex()==1){ %>男<%}else if(accouteVO.getAccSex()==2){ %>女<%}else if(accouteVO.getAccSex()==3){ %>人妖<%} %>
	<%} if (Integer.parseInt(fl) == 2) { %>
	无
	<%} if (Integer.parseInt(fl) == 3) { %>
	<%if(jewelryVO.getJewSex()==1){ %>男<%}else if(jewelryVO.getJewSex()==2){ %>女<%}else if(jewelryVO.getJewSex()==3){ %>人妖<%} %> 
	<%} %>  
	<br/>
	<%if (Integer.parseInt(fl) == 1) {  %>
	...
	是否绑定:
     <%if (Integer.parseInt(fl) == 1) {  %>
	 <%if(accouteVO.getAccBonding()==1){ %>不绑定<%} else if(accouteVO.getAccBonding()==2){%>拾取绑定<%} else if(accouteVO.getAccBonding()==3){%>装备绑定<%} %>
	 <%} if (Integer.parseInt(fl) == 2) { %>
	 <%if(armVO.getArmBanding()==1){ %>不绑定<%}else if(armVO.getArmBanding()==2){ %>拾取绑定<%} %>
	 <%} if (Integer.parseInt(fl) == 3) { %>
	 <%if(jewelryVO.getJewBonding()==1){ %>不绑定<%}else if(jewelryVO.getJewBonding()==2){ %>拾取绑定<%} %> 
	 <%} %> 
	<br/>
	是否保护:
     <%if (Integer.parseInt(fl) == 1) {  %>
