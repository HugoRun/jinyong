...
|<anchor>转让族长<go method="post" href="<%=response.encodeURL(GameConfig.getContextPath()+"/faction.do?cmd=zlList")%>"></go></anchor><br/>
<c:choose>
<c:when test="${faction.isDisband==true }">
<anchor>接管氏族<go method="post" href="<%=response.encodeURL(GameConfig.getContextPath()+"/faction.do?cmd=assume")%>"></go></anchor>
</c:when>
<c:otherwise>
<anchor>解散氏族<go method="post" href="<%=response.encodeURL(GameConfig.getContextPath()+"/faction.do?cmd=disbandHint")%>"></go></anchor>
</c:otherwise>
</c:choose>
<br/>
尊敬的族长大人，你具有以下权力:<br/>
...
7.转让族主，将族主之位转让给各长老之一<br/>
8.解散氏族，将氏族彻底解散<br/>
9.升级氏族，将氏族进行升级（请于祠堂管理页面进行管理）<br/>
10.升级祠堂，对祠堂进行升级（请于祠堂管理页面进行管理）<br/>
