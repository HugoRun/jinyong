<%@page pageEncoding="UTF-8" import="java.util.List,com.ls.ben.vo.mall.CommodityVO,com.ls.pub.bean.QueryPage" %>
<%  
	String cmd = "n1";
	String type = (String)request.getAttribute("type");
	if( type.equals("is_nomral"))
	{
	%><%@ include file="/init/inc_mall/commodity_type.jsp"%><%
		cmd = "nomral";
	}
	if( type.equals("is_hotmall"))
	{
		cmd = "hot";
	}
	if( type.equals("is_jfmall"))
	{
		cmd = "jifenmall";
	}
	if( type.equals("is_vipmall"))
	{
		cmd = "vipmall";
	}
...
//无折扣商品：[购]增气丸一100元宝
%>
[<anchor><go href="<%=response.encodeURL(GameConfig.getContextPath()+"/mall.do?cmd=n6")%>" method="post"><postfield name="c_id" value="<%=commodity.getId()%>"/><postfield name="type" value="<%=type %>"/><postfield name="page_no" value="<%= queryPage.getCurrentPageNo()%>"/></go>购</anchor>]
<% 
...
%>
<anchor>
<go method="post" href="<%=response.encodeURL(GameConfig.getContextPath()+"/mall.do?cmd="+cmd)%>">
<postfield name="type" value="<%=type %>"/>
