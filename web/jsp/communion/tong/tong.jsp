...
<p>
	...
	<go method="get"
		href="<%=response.encodeURL(GameConfig.getContextPath())
					+ "/communioninfoaction.do?cmd=n4"%>" ></go>
	刷新
	</anchor>
	<br />
	<%
		...
		List list = communionService.getTongChatInfoList(roleInfo.getBasicInfo().getFId(), type);
		if (list != null && list.size() != 0) {
			...
			pageb.init(request.getParameter("pageNo"), pageSize, allSize,
					response.encodeURL(GameConfig.getContextPath()
							+ "/communioninfoaction.do?cmd=n4"));
			no = pageb.getIndex();
			for (int i = (no - 1) * pageSize; i < no * pageSize
					&& i < list.size(); i++) {
				CommunionVO vo = (CommunionVO) list.get(i);
				//if(roleInfo.getBasicInfo().getTongId() == vo.getCBang()){
	%>
	[氏]
	<%
