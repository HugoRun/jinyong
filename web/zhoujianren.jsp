        ...
        <p>
            ...
            <%
                if (LangjunConstants.LAST_LANGJUN_PPK != 0) {
                    ...
                    if (re1 != null) {
            %>
            上一轮千面郎君为 ： <%=re1.getBasicInfo().getRealName() %>
            <br/>
            所在场景：<%=re1.getBasicInfo().getSceneInfo().getSceneName() %>
            <br/>

            <%} else {%>
            上一轮千面郎君<%=new RoleService().getName(LangjunConstants.LAST_LANGJUN_PPK + "")[0] %>已下线
            <br/>
            <%} %>
            <%
            } else { %>
            ...
            <%
                if (LangjunConstants.LANGJUN_PPK != 0) {
                    ...
                    if (re != null) {
            %>
            当前的千面郎君为 ： <%=re.getBasicInfo().getRealName() %>
            <br/>
            所在场景：<%=re.getBasicInfo().getSceneInfo().getSceneName() %>
            <br/>

            <%} else {%>
            当前的千面郎君<%=new RoleService().getName(LangjunConstants.LANGJUN_PPK + "")[0] %>已下线
            <br/>
            <%} %>
            <%
            } else { %>
            ...
            <br/>
            千面郎君活动结束时间(格式：2009-09-24 12:12:12):
            <br/>
            <input type="text" name="langjuntime"/>
            <br/>
            <anchor>
                <go method="post" href="<%=response.encodeURL(GameConfig.getContextPath()+"/langjun.do?cmd=n2")%>">
            ...
            迷宫结束时间：<%=DateUtil.getDate(LostConstant.LOST_END_TIME) %>
            <br/>
            <%} %>
            迷宫将于
            ...
            <br/>
            <anchor>
                <go method="post" href="<%=response.encodeURL(GameConfig.getContextPath()+"/langjun.do?cmd=n22")%>">
