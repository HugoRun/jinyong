...
public class UGrowDao extends DaoBase {
    //成长信息缓存
    private static final Map<String, UGrowVO> grow_cache = new HashMap<String, UGrowVO>(300);

    //最高级成长信息
    private static final Map<Integer, UGrowVO> max_grade_grow_cache = new HashMap<Integer, UGrowVO>(2);
    ...
    public UGrowVO getMaxGradeInfo(int race) {
        ...
        conn = dbConn.getConn();
        try {
            String sql = "SELECT SUM(g_HP), SUM(g_MP), SUM(g_gj), SUM(g_fy) FROM `u_grow_info` " +
                    "WHERE `g_grade` = " + max_grade + " AND `g_race` = '" + race + "'";
            logger.debug(sql);
    ...
    public UGrowVO getByGradeAndRace(int grade, int race) {
        ...
        conn = dbConn.getConn();
        try {
            String sql = "SELECT * FROM u_grow_info WHERE g_grade = " + grade + " AND g_race = '" + race + "'";
            logger.debug(sql);
            stmt = conn.createStatement();
            rs = stmt.executeQuery(sql);
            if (rs.next()) {
    ...
    public boolean isAutogrow(int grade, int race) {
        ...
        return growVO.getGIsAutogrow() == 1;
    }
