...
public class PKState extends UserBase {
    ...
    // 其他攻击自己的玩家,最多5个人打一个人
    private final Queue<Integer> other_list = new LinkedList<Integer>();

    // PK时看到的系统消息
    private final List<SystemInfoVO> sys_info_list = new ArrayList<SystemInfoVO>(10);
    ...
    public String getWarningDes() {
        ...
        for (int i = 1; i < other_list.size(); i++) {
            Integer other_ppk = ((LinkedList<Integer>) other_list).get(i);
            RoleEntity other = RoleService.getRoleInfoById(other_ppk);
    ...
    public boolean isZhudongAttackOther(int other_ppk) {
        return this.start_fight_ppk != -1 && other_ppk == this.start_fight_ppk;
    }

    private void put(Integer pPk) {
        if (!other_list.contains(pPk)) {
            other_list.add(pPk);
        }
    }
