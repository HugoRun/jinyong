...
public class RoleEntity implements Serializable {
    ...
    public String getDisplay() {
        String sb = this.getFullName() + "<br/>" +
                "称号:" + getTitleSet().getShowTitleName() + "<br/>" +
                "性别:" + this.getBasicInfo().getSexName() + "<br/>" +
                "等级:" + this.getBasicInfo().getGrade() + "级<br/>" +
                "帮派:" + this.getBasicInfo().getFactionName() + "<br/>" +
                "是否可PK:" + this.getBasicInfo().getPkSwitchDes() + "<br/>" +
                "PK点数:" + this.getBasicInfo().getEvilValue() + "<br/>";
        return sb;
    }
    ...
    public boolean isOnline() {
        return PlayerState.OUTLINE != this.getStateInfo().getCurState();
    }
    ...
    public boolean getIsRookie() {
        return this.getBasicInfo().getPlayer_state_by_new() == 1;
    }
    ...
    public void createEquipProduct(int product_type) {
        switch (product_type) {
            case EquipProduct.upgrade:
                equipProduct = new UpgradeEquip(this.getPPk());
                break;
            case EquipProduct.change_wx:
                equipProduct = new ChangeEquipWX(this.getPPk());
                break;
        }
    }
    ...
    public boolean isPKToOther(RoleEntity other) {
        ...
        if (me_grade >= 30 && Math.abs(me_grade - other_grade) <= 10) {
            ...
            if (me_is_redname || other_is_redname) {
                ...
                if (!me_is_pk_protected)//自己不受PK道具保护
                {
                    boolean other_is_pk_protected = avoidPkPropService.isAvoidPkPropTime(other.getPPk());//other是否有PK道具保护
                    boolean is_same_race = this.getBasicInfo().getPRace() == other.getBasicInfo().getPRace();//是否是相同种族
                    //同种族PK
                    if (is_same_race && cur_scene.isAllowPK() && !other_is_pk_protected && this.getBasicInfo().getPkSwitch() == 2 && other.getBasicInfo().getPkSwitch() == 2) {
                        return true;
                    }
                    //不同种族PK
                    else if (!is_same_race && !other_is_pk_protected) {
                        return true;
                    }
                }
            }
    ...
    public boolean isRedname() {
        return this.getBasicInfo().getEvilValue() >= GameArgs.RED_NAME_VALUE;
    }
