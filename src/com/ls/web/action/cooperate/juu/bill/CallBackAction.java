...
public class CallBackAction extends DispatchAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (sign_bak.equals(sign)) {
            ...
            content = "您于" + time_str + "充值" + accountRecord.getMoney() + "元充值成功，获得【" + GameConfig.getYuanbaoName() + "】×" + yb_num + "！";
            logger.info("邮件标题：" + title);
            logger.info("邮件内容：" + content);
            mailInfoService.sendMailBySystem(accountRecord.getPPk(), title, content);

            gsss.addPropNum(0, StatisticsType.RMB, accountRecord.getMoney(), StatisticsType.DEDAO, Channel.JUU + "", accountRecord.getPPk());// 统计RMB
