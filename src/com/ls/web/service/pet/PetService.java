...
public class PetService {
    ...
    public void petAttackNpcs(Fighter player, List npcs) {
        if (player == null || npcs == null || npcs.isEmpty()) {
            logger.debug("空指针");
            return;
        }
        ...
        {
            petSkillAttackNpc(player, pet, petSkill, npcs, injureMultiple, baojiMultiple);
        } else
        // 发动物理攻击
        {
            petPhysicsAttackNpc(player, pet, petSkill, npcs, injureMultiple, baojiMultiple);
        }
        // 一次攻击改变宠物本身状态如寿命等
    ...
    // 寵物技能攻擊NPC
    private void petSkillAttackNpc(Fighter player, PetInfoVO pet, PetSkillVO petSkill, List npcs, double injureMultiple, double baojiMultiple) {
        ...
        wxInjure = AttackService.getPetWxInjureValue(pet, npc, pet.getPetGrade(), npc.getLevel(), pet_npcId) * 2 / 5;

        int injure = 0;
        int skillInjure = 0;
        int physicsGj1 = 0;
        if (pet.getPetSkillDisplay() != null && pet.getPetSkillDisplay().getPetSkillRound() < 1) {
            physicsGj1 = MathUtil.getRandomBetweenXY((int) (pet.getPetGjXiao() * (1 + injureMultiple) * (1 + pet.getPetSkillDisplay().getPetSkillMultiple())), (int) (pet.getPetGjDa() * (1 + injureMultiple) * (1 + pet.getPetSkillDisplay().getPetSkillMultiple())));
        } else {
            physicsGj1 = MathUtil.getRandomBetweenXY((int) (pet.getPetGjXiao() * (1 + injureMultiple)), (int) (pet.getPetGjDa() * (1 + injureMultiple)));
        }// 得到宠物的最终攻击力
        int physicsGj = physicsGj1 * 2;
        player.setPetSkillDisplay(StringUtil.isoToGBK(petSkill.getPetSkillName()));
        if (petSkill.getPetSkillType() == 5) {
            player.setPetSkillDisplay(StringUtil.isoToGBK(petSkill.getPetSkillName()) + ",三回合内攻击力提高" + petSkill.getPetSkillMultiple() * 100 + "%");
        }
        int skillGj = MathUtil.getRandomBetweenXY(petSkill.getPetSkillGjXiao(), petSkill.getPetSkillGjDa());
        if (petSkill.getPetSkillArea() == 1)// 群体攻击
        {
            ...
            for (int i = npcs.size() - 1; i >= 0; i--) {
                ...
                // 修改技能伤害公式
                skillInjure = AttackService.petSkillInjureExpressions(skillGj, pet.getPetGrade(), pet.getPetGrow(), npc.getNpcDefance(), npc.getLevel(), physicsGj);
                ...
                npcService.updateNPCCurrentHP(player, npc, injure, NpcService.PETINJURE);
                if (npc.isDead()) {
            ...
            if (petSkill.getPetSkillSeveral() != 1)// 判断攻击次数
            {
                ...
                for (int x = 0; x < petSkill.getPetSkillSeveral(); x++) {
                    skillGj = MathUtil.getRandomBetweenXY(petSkill.getPetSkillGjXiao(), petSkill.getPetSkillGjDa());
                    skillInjure = AttackService.petSkillInjureExpressions(skillGj, pet.getPetGrade(), pet.getPetGrow(), npc.getNpcDefance(), npc.getLevel(), physicsGj);
                    injure = (int) (skillInjure * petSkill.getPetSkillMultiple() / 2);
                    logger.info("宠物技能伤害:" + skillInjure);
                    if (injure <= 0) {
                        injure = 1;
                    }
        ...
        if (petSkill.getPetSkillType() == 4) {
            ...
            pet.getPetSkillDisplay().setPetSkillMultiple(petSkill.getPetSkillMultiple());
        } else {
            ...
            int round = pet.getPetSkillDisplay().updateSkillRound(pet.getPetSkillDisplay().getPetSkillRound());
            pet.getPetSkillDisplay().setPetSkillRound(round);
        }
    }

    // 宠物物理攻擊NPC
    private void petPhysicsAttackNpc(Fighter player, PetInfoVO pet, PetSkillVO petSkill, List npcs, double injureMultiple, double baojiMultiple) {
        NpcService npcService = new NpcService();
        NpcFighter npc = (NpcFighter) npcs.get(0);
        ...
        int physicsGj = physicsGj1 * 2;
        physicsInjure = AttackService.normalInjureExpressions(physicsGj, npc.getNpcDefance(), pet.getPetGrade(), npc.getLevel());
        if (petSkill != null && petSkill.getPetSkillType() == 0 && MathUtil.PercentageRandomByParamdouble((pet.getPetViolenceDrop() + baojiMultiple), 1))// 得到暴击率
        // 并判断是否产生暴击
        {
    ...
    public void petAttackPlayer(Fighter playerA, Fighter playerB, String tong) {
        ...
        wxInjure = AttackService.getPetWxInjureValue(pet, playerB.getWx(), pet.getPetGrade(), playerB.getPGrade(), pet_npcId) * 2 / 5;

        PetSkillVO petSkill = getPetSkill(pet);
        if (petSkill != null)// 发动技能攻击
        {
            petSkillAttackPK(pet, playerA, playerB, petSkill, tong, wxInjure, baojiMultiple, injureMultiple);
        } else
        // 发动物理攻击
        {
            petPhysicsAttackPK(pet, playerA, playerB, petSkill, tong, wxInjure, baojiMultiple, injureMultiple);
        }
    ...
    // 寵物技能攻擊
    private void petSkillAttackPK(PetInfoVO pet, Fighter playerA, Fighter playerB, PetSkillVO petSkill, String tong, int wxInjure, double baojiMultiple, double injureMultiple) {
        ...
        playerA.setPetSkillDisplay(StringUtil.isoToGBK(petSkill.getPetSkillName()));
        if (petSkill.getPetSkillType() == 5) {
            playerA.setPetSkillDisplay(StringUtil.isoToGBK(petSkill.getPetSkillName()) + ",三回合内攻击力提高" + petSkill.getPetSkillMultiple() * 100 + "%");
        }
        int skillGj1 = MathUtil.getRandomBetweenXY((int) (pet.getPetGjXiao() * (1 + injureMultiple) + petSkill.getPetSkillGjXiao()), (int) (pet.getPetGjDa() * (1 + injureMultiple) + petSkill.getPetSkillGjDa()));
        // 5月5号 宠物PK 攻击x4
        ...
        if (petSkill.getPetSkillSeveral() != 1)// 判断攻击次数
        {
            ...
            for (int x = 0; x < petSkill.getPetSkillSeveral(); x++) {
                ...
                skillGj = MathUtil.getRandomBetweenXY((int) (petSkill.getPetSkillGjXiao() * (1 + injureMultiple) + petSkill.getPetSkillGjXiao()), (int) (petSkill.getPetSkillGjDa() * (1 + injureMultiple) + petSkill.getPetSkillGjDa()));
                int skillGj_temp = skillGj + physicsGj * 60 / 100;

                skillInjure = AttackService.normalInjureExpressions(skillGj_temp, playerB.getFy(), pet.getPetGrade(), playerB.getPGrade());
                ...
                logger.info("宠物发动技能攻击技能,技能伤害=" + injure + " ,wxInjure=" + wxInjure);
            ...
            skillInjure = AttackService.normalInjureExpressions(skillGj * 80 / 100, playerB.getFy(), pet.getPetGrade(), playerB.getPGrade());
            injure = (int) (skillInjure * petSkill.getPetSkillMultiple()) / 3;
            if (injure <= 0) {
                injure = 1;
            }
            ...
            fightService.statUpdateByPetInjure(playerB, playerA, injure, tong, -1);
            logger.debug("宠物发动技能攻击技能,技能伤害=" + injure + " ,wxInjure=" + wxInjure);
        ...
        if (petSkill.getPetSkillType() == 4) {
            ...
            if (petSkill.getPetSkillType() == 3) {
                injure = injure / 2;
                playerA.setPlayerInjure(-injure);
    ...
    // 寵物普通攻擊
    private void petPhysicsAttackPK(PetInfoVO pet, Fighter playerA, Fighter playerB, PetSkillVO petSkill, String tong, int wxInjure, double baojiMultiple, double injureMultiple) {
        FightService fightService = new FightService();
        int physicsInjure = 0;
        int injure = 0;
        int physicsGj = 0;
        if (pet.getPetSkillDisplay() != null && pet.getPetSkillDisplay().getPetSkillRound() < 1) {
            physicsGj = MathUtil.getRandomBetweenXY((int) (pet.getPetGjXiao() * (1 + injureMultiple) * (1 + pet.getPetSkillDisplay().getPetSkillMultiple())), (int) (pet.getPetGjDa() * (1 + injureMultiple) * (1 + pet.getPetSkillDisplay().getPetSkillMultiple())));
        } else {
            physicsGj = MathUtil.getRandomBetweenXY((int) (pet.getPetGjXiao() * (1 + injureMultiple)), (int) (pet.getPetGjDa() * (1 + injureMultiple)));
        }
        // 5月5号 宠物PK 攻击x4
        physicsInjure = AttackService.normalInjureExpressions(physicsGj * 4, playerB.getFy(), pet.getPetGrade(), playerB.getPGrade());
        if (petSkill != null && petSkill.getPetSkillType() == 0 && MathUtil.PercentageRandomByParamdouble((pet.getPetViolenceDrop() + baojiMultiple), 1)) {
            ...
            logger.info("宠物发动普通攻击技能，普通伤害(双倍)=" + injure + " ,wxInjure=" + wxInjure * 2);
        } else {
            ...
            logger.debug("宠物发动普通攻击技能，普通伤害=" + injure + " ,wxInjure=" + wxInjure);
        }
    ...
    public PetSkillVO getPetSkill(PetInfoVO pet) {
        ...
        if (petSkillCache.getPetSkillType(pet.getPetSkillOne()) != 0)// 判断技能类型是否为主动技能
        {
            ...
            petControl = petControlDao.getByPetAndSkillGroup(pet.getPetId(), pet_skill_gourp);
        ...
        if (petControl == null) {
            ...
            petSkill = petSkillCache.getPetSkillById(petControl.getPetSkillId());
            return petSkill;
        }
    }
    ...
    public List<Double> getPetPassSkillAttribute(PetInfoVO pet) {
        ...
        List<PetSkillVO> skilllist = dao.getPassSkill(pet.getPetSkillOne(), pet.getPetSkillTwo(), pet.getPetSkillThree(), pet.getPetSkillFour(), pet.getPetSkillFive());
     ...
     * @return
     */
    public PetSkillControlVO getSkillByControl(List<PetSkillControlVO> petControls) {
    ...
    private void initPet(int pet_pk) {
        ...
        // 得到宠物表
        PetShapeVO petShapeVO = petDAO.getPetShapeView(player_pet.getPetType(), 1);// 通过宠物等级和宠物ID和宠物类型 去找宠物成长信息
        ...
        {
            grow_quality++;
            pet_grow = getGrowByQuality(pet.getPetDropXiao(), pet.getPetDropDa());
        } else if (player_pet.getPetInitNum() > 0) {
            pet_grow = getGrowByQuality(pet.getPetDropXiao(), pet.getPetDropDa());
        }
        DecimalFormat df = new DecimalFormat("0.00");
        pet_grow_str = df.format(pet_grow);

        pet_xia_exp = (int) (Integer.parseInt(petShapeVO.getShapeXiaExperience().trim()) * pet_grow);
         ...
         */
        if (pet.getSkillControl() != 0) { // 只有在宠物拥有技能数不为零时需要计算获得何种技能

            int i = 0;
            int skill_num = 0;
            List<Integer> list = new ArrayList<Integer>();
            if (pet.getPetSkillOne() != 0) list.add(pet.getPetSkillOne());
            if (pet.getPetSkillTwo() != 0) list.add(pet.getPetSkillTwo());
            if (pet.getPetSkillThree() != 0) list.add(pet.getPetSkillThree());
            if (pet.getPetSkillFour() != 0) list.add(pet.getPetSkillFour());
            if (pet.getPetSkillFive() != 0) list.add(pet.getPetSkillFive());
            ...
            if (list.size() != 0) {
                ...
                if (MathUtil.isAppearByPercentage(50, 100)) {
                    if (skill_num < pet.getSkillControl()) { // 在已有的技能数小于宠物所应有的技能情况下,才能继续.
                        do {
                            i = random.nextInt(list.size());
                            pet_skill_two = list.get(i);
                            logger.info("第2种技能50%可能已经产生附着");
                            skill_num++;
                        } while (pet_skill_two == pet_skill_one || pet.getSkillControl() == 1);
                    }
                }
                if (MathUtil.PercentageRandomByParamdouble(10, 100)) {
                    if (skill_num < pet.getSkillControl()) { // 在已有的技能数小于宠物所应有的技能情况下,才能继续.
                        do {
                            ...
                            skill_num++;
                        } while (pet_skill_three == pet_skill_one || pet_skill_three == pet_skill_two);
                    }
                }
                if (MathUtil.PercentageRandomByParamdouble(1, 100)) {
                    if (skill_num < pet.getSkillControl()) { // 在已有的技能数小于宠物所应有的技能情况下,才能继续.
                        do {
                            ...
                            skill_num++;
                        } while (pet_skill_four == pet_skill_one || pet_skill_four == pet_skill_two || pet_skill_four == pet_skill_three);
                    }
                }
                if (MathUtil.PercentageRandomByParamdouble(0.1, 100)) {
                    if (skill_num < pet.getSkillControl()) { // 在已有的技能数小于宠物所应有的技能情况下,才能继续.
                        do {
                            ...
                            skill_num++;
                        } while (pet_skill_five == pet_skill_one || pet_skill_five == pet_skill_two || pet_skill_five == pet_skill_three || pet_skill_five == pet_skill_four);
                    }
                }
            }
        }
        ...
        if (player_pet.getPetName().contains("*")) {
            ...
            if (pet_skill_one != 0 || pet_skill_two != 0 || pet_skill_three != 0 || pet_skill_four != 0 || pet_skill_five != 0) {
                xing = "*";
            }
        }
        petInfoDao.initPet(pet_pk, pet_grow_str, pet_xia_exp, gj_xiao, gj_da, pet_sale, pet_skill_one, pet_skill_two, pet_skill_three, pet_skill_four, pet_skill_five, xing);
    ...
    public int getGrowQuality(double pet_grow) {
        ...
        {
            grow_quality = 0;
        } else if (pet_grow >= 1.6 && pet_grow <= 2.0)// 上品
        {
            grow_quality = 1;
        } else if (pet_grow >= 2.1 && pet_grow <= 2.5)// 极品
        {
            grow_quality = 2;
        }
        return grow_quality;
    ...
    public String createPetByPetID(int p_pk, int pet_id) {
        ...
        PetDAO petDAO = new PetDAO();
        PetVO pet = petDAO.getPetViewByPetID(pet_id);
        if (pet != null) {
            ...
            if (MathUtil.PercentageRandomByParamdouble(0.3, 100)) {
                if (skill_num < pet.getSkillControl()) { // 在已有的技能数小于宠物所应有的技能情况下,才能继续.
                    do {
                        ...
                        logger.info("第3种技能0.3%可能已经产生附着");
                        skill_num++;
                    } while (petSkillThree == petSkillOne || petSkillThree == petSkillTwo);
                }
            }
            if (MathUtil.PercentageRandomByParamdouble(0.03, 100)) {
                if (skill_num < pet.getSkillControl()) { // 在已有的技能数小于宠物所应有的技能情况下,才能继续.
                    do {
                        ...
                        logger.info("第4种技能0.03%可能已经产生附着");
                        skill_num++;
                    } while (petSkillFour == petSkillOne || petSkillFour == petSkillTwo || petSkillFour == petSkillThree);
                }
            }
            ...
            if (MathUtil.PercentageRandomByParamdouble(0.003, 100)) {
                if (skill_num < pet.getSkillControl()) { // 在已有的技能数小于宠物所应有的技能情况下,才能继续.
                    ...
                    do {
                        ...
                        logger.info("第5种技能0.003%可能已经产生附着");
                    } while (petSkillFive == petSkillOne || petSkillFive == petSkillTwo || petSkillFive == petSkillThree || petSkillFive == petSkillFour);
                }
            }
            ...
            double pet_grow = MathUtil.getRandomDoubleXY(petDropXiao, petDropDa);
            ...
            PetShapeVO petShapeVO = petDAO.getPetShapeView(pet.getPetType(), 1);
            if (petShapeVO == null) {
                hint = "成长表错误";
                return hint;
            }
            ...
            String petExp = "0";
            /** 下级经验达到下一级需要的经验 */
            int petXiaExps = (int) (Integer.parseInt(petShapeVO.getShapeXiaExperience().trim()) * pet_grow);
            ...
            PetInfoDAO petInfoDAO = new PetInfoDAO();
            petInfoDAO.getPetInfoAdd(pPk, petId, petName, petNickname, petGrade, petExp, petXiaExp, petGjXiao, petGjDa, petSale, pet_grow_str, petWx, petWxValue, petSkillOne, petSkillTwo, petSkillThree, petSkillFour, petSkillFive, petLife, petType, petIsBring, petFatigue, petLonge, longeNumber, longeNumberOk, skillControl, pet.getPetType(), petPic, pet.getPetViolenceDorp());
            hint = petName;
        } else {
            hint = "创建宠物失败";
        }
    ...
    public String getPetDisplayWml(PetInfoVO pet) {
        ...
        resultWml.append(picService.getPetPicStr(roleInfo, pet.getPetPk() + "") + StringUtil.isoToGBK(pet.getPetNickname()) + "<br/>");
        // resultWml.append(picService.getPetPicStr(Math.abs(pet.getPPk())+"",
        // pet.getPetPk()+""));
        resultWml.append("宠物:" + StringUtil.isoToGBK(pet.getPetName()) + "<br/>");
        resultWml.append("状态:");
        ...
        /** ***********TODO************************* */
        resultWml.append("经验:" + pet.getPetBenExp() + "/" + pet.getPetXiaExp() + "<br/>");
        if (petSkillCache.getInjureMultiple(pet.getPetSkillOne()) != 0 || petSkillCache.getInjureMultiple(pet.getPetSkillTwo()) != 0 || petSkillCache.getInjureMultiple(pet.getPetSkillThree()) != 0 || petSkillCache.getInjureMultiple(pet.getPetSkillFour()) != 0 || petSkillCache.getInjureMultiple(pet.getPetSkillFive()) != 0) {
            resultWml.append("攻击:" + (Integer.valueOf((int) (pet.getPetGjXiao() * (1 + petSkillCache.getInjureMultiple(pet.getPetSkillOne()) + petSkillCache.getInjureMultiple(pet.getPetSkillTwo()) + petSkillCache.getInjureMultiple(pet.getPetSkillThree()) + petSkillCache.getInjureMultiple(pet.getPetSkillFour()) + petSkillCache.getInjureMultiple(pet.getPetSkillFive()))))) + "-" + Integer.valueOf((int) (pet.getPetGjDa() * (1 + petSkillCache.getInjureMultiple(pet.getPetSkillOne()) + petSkillCache.getInjureMultiple(pet.getPetSkillTwo()) + petSkillCache.getInjureMultiple(pet.getPetSkillThree()) + petSkillCache.getInjureMultiple(pet.getPetSkillFour()) + petSkillCache.getInjureMultiple(pet.getPetSkillFive())))) + "<br/>");
        } else {
        ...
        if (pet.getPetSkillOne() == 0 && pet.getPetSkillTwo() == 0 && pet.getPetSkillThree() == 0 && pet.getPetSkillFour() == 0 && pet.getPetSkillFive() == 0) {
            ...
            resultWml.append("技能:" + petSkillCache.getName(pet.getPetSkillOne()) + petSkillCache.getName(pet.getPetSkillTwo()) + petSkillCache.getName(pet.getPetSkillThree()) + petSkillCache.getName(pet.getPetSkillFour()) + petSkillCache.getName(pet.getPetSkillFive()) + "<br/>");
        }
        resultWml.append("寿命:" + pet.getPetLonge() + "/" + pet.getPetLife() + "<br/>");
    ...
    public void petSale(String pPk, String petPk) {
        ...
        logService.recordMoneyLog(roleInfo.getBasicInfo().getPPk(), roleInfo.getBasicInfo().getName(), roleInfo.getBasicInfo().getCopper() + "", petPrice + "", "卖宠物");
    ...
    public void addSkillFlag(String petpk) {
        ...
        if (infovo.getPetName().contains("*")) {
            ...
            if (infovo.getPetName().equals(infovo.getPetNickname())) {
                infodao.updateNickName(Integer.parseInt(petpk), infovo.getPetNickname() + "*");
            }
        }
    ...
    // 得到宠物技能的描述
    public String getPetSkillDisplay(PetInfoVO pet, int skill_id, String skill_name, HttpServletRequest request, HttpServletResponse response) {
        if (skill_name.equals("")) {
            return "";
        } else {
            String resultWml = "<anchor> " +
                    "<go method=\"post\" href=\"" + response.encodeURL(GameConfig.getContextPath() + "/petinfoaction.do") + "\">" +
                    "<postfield name=\"cmd\" value=\"n7\" /> " +
                    "<postfield name=\"petPk\" value=\"" + pet.getPetPk() + "\" /> " +
                    "<postfield name=\"petGrade\" value=\"" + pet.getPetGrade() + "\" /> " +
                    "<postfield name=\"petFatigue\" value=\"" + pet.getPetFatigue() + "\" /> " +
                    "<postfield name=\"petIsBring\" value=\"" + pet.getPetIsBring() + "\" /> " +
                    "<postfield name=\"skill_id\" value=\"" + skill_id + "\" /> " +
                    "</go>" +
                    skill_name +
                    "</anchor> ";
            return resultWml;
     ...
     * @return
     */
    public String getPetDisplayWmlOwn(PetInfoVO pet, HttpServletRequest request, HttpServletResponse response) {
        PetSkillCache petSkillCache = new PetSkillCache();
        PicService picService = new PicService();
        if (pet == null) {
            logger.info("参数错误");
        ...
        if (pet.getPetSkillOne() == 0 && pet.getPetSkillTwo() == 0 && pet.getPetSkillThree() == 0 && pet.getPetSkillFour() == 0 && pet.getPetSkillFive() == 0) {
            ...
            String skill = "";
            if (pet.getPetSkillOne() != 0) {
                skill = skill + getPetSkillDisplay(pet, pet.getPetSkillOne(), petSkillCache.getName(pet.getPetSkillOne()), request, response);
            }
            if (pet.getPetSkillTwo() != 0) {
                skill = skill + getPetSkillDisplay(pet, pet.getPetSkillTwo(), petSkillCache.getName(pet.getPetSkillTwo()), request, response);
            }
            if (pet.getPetSkillThree() != 0) {
                skill = skill + getPetSkillDisplay(pet, pet.getPetSkillThree(), petSkillCache.getName(pet.getPetSkillThree()), request, response);
            }
            if (pet.getPetSkillFour() != 0) {
                skill = skill + getPetSkillDisplay(pet, pet.getPetSkillFour(), petSkillCache.getName(pet.getPetSkillFour()), request, response);
            }
            if (pet.getPetSkillFive() != 0) {
                skill = skill + getPetSkillDisplay(pet, pet.getPetSkillFive(), petSkillCache.getName(pet.getPetSkillFive()), request, response);
            }
    ...
    public String getPetDisplayWmlOwn(int pet_pk, HttpServletRequest request, HttpServletResponse response) {
        PetInfoDao petInfoDao = new PetInfoDao();
        PetInfoVO pet = petInfoDao.getPet(pet_pk);
