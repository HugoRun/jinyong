...
public class NpcService
{
		...
		for (int i = 0; i < npcs.size(); i++)
		{
			...
			if(npc.getNpcType() == NpcAttackVO.NIANSHOU){
				...
				if (player.isDead()) // 如果死了伤害为
				{
					...
					player.appendKillDisplay("您被"
							+ StringUtil.isoToGBK(npc.getNpcName()) + "杀死了！");
					if (i != 0)
					{
	...
	private List<DropGoodsVO> dropGoodsByGoldBox(int npc_id, PartInfoVO player)
	{
		...
		List<NpcdropVO> npcdrops = npcDropDao.getNpcdropsByNpcID(npc_id,
				drop_task_condition); // npc所有要掉的物品
	...
	private void addGoodsToDropTable(NpcdropVO npcdropVO, PartInfoVO player,List<DropGoodsVO> drop_goods_list)
	{
		...
		if (npcdropVO != null )
		{
					...
					if ( dropGoodsVO.getGoodsImportance() == 2) {
						GoodsService goodsService = new GoodsService();
						String dropInfo = "恭喜"+player.getPName()+"打死"+NpcCache.getNpcNameById(npcdropVO.getNpcID())+"获得"
						...
						GoodsService goodsService = new GoodsService();
						String dropInfo = "恭喜"+player.getPName()+"打开"+NpcCache.getNpcNameById(npcdropVO.getNpcID())+"获得"
									+goodsService.getGoodsName(npcdropVO.getGoodsId(), npcdropVO.getGoodsType());
						dropGoodsVO.setDropGoodsInfo(dropInfo);
			...
			for (NpcdropVO npcdropVO : npcdrops)
			{
				...
				if ( (isDrop && drop_goods_list.size() < maxSize) || (npcdropVO.getNpcdropProbability() >= MathUtil.DROPDENOMINATOR) )
				{
					...
					//打怪得到的物品(特殊)
						...
						GoodsService goodsService = new GoodsService();
						String dropInfo = "恭喜"+player.getPName()+"奴役"+NpcCache.getNpcNameById(npcdropVO.getNpcID())+"获得"
									+goodsService.getGoodsName(npcdropVO.getGoodsId(), npcdropVO.getGoodsType());
						dropGoodsVO.setDropGoodsInfo(dropInfo);
					}
...
/*	// 如果此是npc是雕像类, 就处理雕像类npc死亡后,攻城战场的相关情况
	...
	*/
			...
			int murderer_type)
	{
		...
		if (npc.isDead())
		{
			...
			if (npc.getNpcType() == NpcAttackVO.DEADNPC)
			{
				...
				{
					player.appendKillDisplay(player.getKillDisplay() + "您打败了"
			...
			int npcRefurbishTime)
	{
		...
		sb.append(df.format(new Date())).append(StringUtil.gbToISO("分,"))
				.append(npc.getNpcName()).append(StringUtil.gbToISO("在"))
				.append(sceneStr).append(StringUtil.gbToISO("被")).append(
						player.getPName()).append(StringUtil.gbToISO("杀死！"));
		systemInfoService.insertSystemInfoBySystem(sb.toString());
		// 发送系统告知玩家, boss将于五分钟后刷新
		StringBuffer sb1 = new StringBuffer();
		sb1.append(npc.getNpcName()).append(StringUtil.gbToISO("将于五分钟后在"))
				.append(sceneStr).append(StringUtil.gbToISO("出现"));
		systemInfoService.insertSystemInfoBySystem(sb1.toString(), npc
				.getNpcRefurbishTime() - 5);
		// 发送系统消息告知玩家 ,boss已经开始刷新
		StringBuffer sb2 = new StringBuffer();
		sb2.append(npc.getNpcName()).append(StringUtil.gbToISO("在")).append(
				sceneStr).append(StringUtil.gbToISO("现在出现了！"));
	...
	public void sendSysInfoOfCatch(int pPk, NpcAttackVO npc)
	{
		...
		sb.append(df.format(new Date())).append(StringUtil.gbToISO("分,"))
				.append(npc.getNpcName()).append(StringUtil.gbToISO("在"))
				.append(sceneStr).append(StringUtil.gbToISO("被")).append(pName)
				.append(StringUtil.gbToISO("捕捉！"));
		systemInfoService.insertSystemInfoBySystem(sb.toString());
		// 发送系统告知玩家, boss将于五分钟后刷新
		StringBuffer sb1 = new StringBuffer();
		sb1.append(npc.getNpcName()).append(StringUtil.gbToISO("将于五分钟后在"))
	...
	public String getWxNum(int npc_id)
	{
		...
		if (vo1.getNpcskiWx() != 0)
		{
			...
			{
				out = "金" + vo1.getNpcskiWxInjure() + "-"
						+ vo2.getNpcskiWxInjure();
				return out;
			}
			if (vo1.getNpcskiWx() == 2)
			{
				out = "木" + vo1.getNpcskiWxInjure() + "-"
						+ vo2.getNpcskiWxInjure();
				return out;
			}
			if (vo1.getNpcskiWx() == 3)
			{
				out = "水" + vo1.getNpcskiWxInjure() + "-"
						+ vo2.getNpcskiWxInjure();
				return out;
			}
			if (vo1.getNpcskiWx() == 4)
			{
				out = "火" + vo1.getNpcskiWxInjure() + "-"
						+ vo2.getNpcskiWxInjure();
				return out;
			}
			if (vo1.getNpcskiWx() == 5)
			{
				out = "土" + vo1.getNpcskiWxInjure() + "-"
						+ vo2.getNpcskiWxInjure();
				return out;
			}
