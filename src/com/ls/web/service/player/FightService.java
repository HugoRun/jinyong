...
public class FightService {
    ...
    public void physicsAttackPlayer(Fighter playerA, Fighter playerB) {
        ...
        if (MathUtil.PercentageRandomByParamdouble((playerA.getDropMultiple()), 100)) {
    ...
    private boolean loadRandAttackSkill(Fighter playerA) {
        ...
        if (skillList != null && skillList.size() != 0) {
            int rand_index = MathUtil.getRandomBetweenXY(0, skillList.size() - 1);
    ...
    public void autoCounterattack(Fighter playerA, Fighter playerB) {
        ...
        if (is_successed)// 玩家有技能发动，使用技能攻击
        {
            skillAttackPlayer(playerA, playerB);
        } else {
            physicsAttackPlayer(playerA, playerB);
    ...
    public void skillAttackNPC(Fighter player, List npcs) {
        ...
        {
            if (use_display == null) {
                playerSkillDao.updateUsetimeAndSleight(playerSkill.getPPk(), playerSkill.getSkId());
            }
        } else {
            playerSkillDao.updateUsetimeAndSleight(playerSkill.getPPk(), playerSkill.getSkId());
        }
        ...
        wxInjure = AttackService.getWxInjureValue(player.getWx(), npc, player.getPGrade(), npc.getLevel(), player.getPPk());// * 10;

        int skillGj = (playerSkill.getSkDamage() + player.getGj());
        logger.info("************技能攻击: 属性攻击和装备攻击:" + player.getGj() + ";技能攻击:" + skillGj);

        player.setSkillDisplay(playerSkill.getSkName());
        boolean isAttackDizzyNPC = MathUtil.isAppearByPercentage(playerSkill.getSkYun());

        boolean isHaveBuffEffect = false;
        BuffEffectService buffEffectService = new BuffEffectService();
        if (playerSkill.getSkBuff() != 0 && MathUtil.isAppearByPercentage(playerSkill.getSkBuffProbability())) {
        ...
        if (playerSkill.getSkArea() == 1) // 群体攻击
        {
            ...
            for (int i = npcs.size() - 1; i >= 0; i--) {
                ...
                if (MathUtil.PercentageRandomByParamdouble((player.getDropMultiple()), 100)) {
                    skillInjure = AttackService.normalInjureExpressions(skillGj, npc.getNpcDefance(), player.getPGrade(), npc.getLevel());
                    injure = (skillInjure + wxInjure) * 2 + 20;
                    // 显示效果
                    player.setSkillDisplay(playerSkill.getSkName() + "攻击,产生了暴击效果!");
                } else {
                    skillInjure = AttackService.normalInjureExpressions(skillGj, npc.getNpcDefance(), player.getPGrade(), npc.getLevel());
                ...
                npcService.updateNPCCurrentHP(player, npc, injure, NpcService.PLAYERINJURE);
                if (npc.isDead()) {
                    if (npcs != null && npcs.size() != 0) {
                        npcs.remove(0);
                    }

                    if (npc.getNpcType() == NpcAttackVO.CITYDOOR || npc.getNpcType() == NpcAttackVO.DIAOXIANG) {
                        AttacckCache attacckCache = new AttacckCache();
                        attacckCache.clearNpcBySceneId(npc.getSceneId() + "");
                    }
                } else {
                    if (isHaveBuffEffect) {
                        buffEffectService.createBuffEffect(npc.getID(), BuffSystem.NPC, playerSkill.getSkBuff());
                    }
                }
        ...
        // 单体攻击
        {
            ...
            if (npc.isDead()) {
                ...
                if (npc.getDizzyBoutNum() == 0 && isAttackDizzyNPC) {
                    ...
                    npcService.updateDizzyBoutNumOfNPC(npc.getID(), playerSkill.getSkYunBout());
                }
    ...
    public void physicsAttackNPC(Fighter player, List npcs) {
        ...
        // 五行伤害
        wxInjureValue = AttackService.getWxInjureValue(player.getWx(), npc, player.getPGrade(), npc.getLevel(), player.getPPk());// * 10;
        // 物理伤害
        ...
        if (MathUtil.PercentageRandomByParamdouble((player.getDropMultiple()), 100)) {
            ...
            npcService.updateNPCCurrentHP(player, npc, injureValue, NpcService.PLAYERINJURE);
            if (npc.isDead()) {
                if (npcs != null && npcs.size() != 0) {
                    npcs.remove(0);
                }
     ...
     *
     * @param player
     * @param exp_of_role 角色得到的原始经验
     * @param exp_of_pet  宠物得到的原始经验
     * @return 结果为result[0]表示角色得到的总的经验，result[1]表示宠物得到的总的经验
     */
    private int[] getFigthWinExp(PartInfoVO player, int exp_of_role, int exp_of_pet) {
        ...
        {
            groupEffectValue = groupService.getGroupEffectPercent(p_pk, scene_id);// 得到组队效果值
        }
        ...
        int role_buff_exp_append = (int) (1.0 * exp_of_role * player.getAppendExp() / 100); // buff经验加成.
        // int role_off_line_buff_exp =
        // roleBeOffBuffService.getRoleOffLineExp(p_pk, exp_of_role);//得到玩家的离线经验

        total_exp_of_role = (int) (1.0 * (exp_of_role + role_sys_exp_append + role_buff_exp_append) * (100 + groupEffectValue) / 100);

        logger.info("角色原始经验：" + exp_of_role + ";系统加成经验:" + role_sys_exp_append + ";buff加成经验:" + role_buff_exp_append + ";");
        logger.info("角色总经验:" + total_exp_of_role);
        // ********************计算玩家所得总的经验结束

        // ********************计算宠物所得总的经验
        int pet_buff_exp_append = (int) (1.0 * exp_of_pet * player.getAppendPetExp() / 100); // buff经验加成

        total_exp_of_pet = (int) (1.0 * (exp_of_pet + pet_buff_exp_append) * (100 + groupEffectValue) / 100); // 总经验
    ...
    public PartInfoVO playerWIN(PartInfoVO player, FightList winList) {
        ...
        int[] exp_result = getFigthWinExp(player, dropExpMoney.getDropExp(), dropExpMoney.getDropPetExp());
        ...
        if (my.isTeaOnline(p_pk)) {
            total_exp_of_role = total_exp_of_role * ShituConstant.MORE_EXP / 100;
        }
        ...
        // 增加经验,玩家成长
        String grow_display = growService.playerGrow(role_info, player, total_exp_of_role);
        ...
        // 执行统计
        gsss.addPropNum(6, StatisticsType.MONEY, dropExpMoney.getDropMoney(), StatisticsType.DEDAO, StatisticsType.DAGUAI, p_pk);
    ...
    private PartInfoVO deadByPK(Fighter loser, Fighter winner) {
        ...
        //****************************死亡描述
        if (!StringUtils.isEmpty(consume_equip_endure_des)) {
     ...
     * @return
     */
    public boolean statUpdateByPetInjure(Fighter playerB, Fighter playerA, int injure, String tong, int npc_type) {
    ...
    private int addPkValue(Fighter winner, Fighter loser) {
        ...
        if (!winner_info.getPKState().isZhudongAttackOther(loser.getPPk()))//只有主动攻击方才增加罪恶值
        {
            return 0;
        }
