...
public class GrowService {
    ...
    public long dropExperience(PartInfoVO player) {
        ...
        // pk为30级以下不掉经验
        if (player.getPGrade() <= 29) return 0;
    ...
    public long dropPlayerExperience(PartInfoVO player) {
        // 洪荒为10级以下不掉经验
        if (player.getPGrade() <= 10) return 0;
        long dropedExp = 0;
        if (player == null) {
            return 0;
        }
     ...
     */
    public String playerGrow(RoleEntity roleEntity, PartInfoVO player, long experience) {

        String grow_display = null;
        PartInfoVO character = player;
        long current_experience = Long.parseLong(character.getPExperience().trim());// 当前经验值
        long xia_experience = Long.parseLong(character.getPXiaExperience().trim());// 下一级所需经验值
        ...
        if (player.getPGrade() == GameConfig.getGradeUpperLimit())// 如果到最高级则不升级
        {
            ...
            if (last_experience > xia_experience)// 当前经验已到升级经验要求
            {
                ...
                UGrowDao growDao = new UGrowDao();
                boolean isAutoGrow = growDao.isAutogrow(character.getPGrade(), character.getPRace());
        ...
        logService.recordExpLog(roleEntity.getBasicInfo().getPPk(), roleEntity.getBasicInfo().getName(), roleEntity.getBasicInfo().getCurExp(), experience + "", "打怪得到");

        // 统计需要
        new RankService().updateAdd(roleEntity.getBasicInfo().getPPk(), "p_exp", experience);
        // 给玩家增加经验
     ...
     * @param roleEntity
     * @param experience
     * @param type       为1代表有返回,为0代表返回null值
     * @return
     */
    private String maxGradeDoThing(RoleEntity roleEntity, long experience, int type) {
        String maxGradeDisplay = null;
        if (roleEntity.getBasicInfo().getGrade() == GameConfig.getGradeUpperLimit() && roleEntity.getBasicInfo().getCurExp().equals(roleEntity.getBasicInfo().getNextGradeExp())) {
            int translateExp = Math.round((float) experience / 1000);
            roleEntity.getBasicInfo().addCopper(translateExp);

            maxGradeDisplay = "由于您已经到达最高级,所以所得经验的千分之一转化为金钱!<br/>获得" + translateExp + "灵石";
        }
     ...
     */
    public String playerGrowTask(RoleEntity roleEntity, PartInfoVO player, int experience) {
        String grow_display = null;
        PartInfoVO character = player;
        ...
        // 监控
        LogService logService = new LogService();
        logService.recordExpLog(roleEntity.getBasicInfo().getPPk(), roleEntity.getBasicInfo().getName(), roleEntity.getBasicInfo().getCurExp(), experience + "", "任务得到");
    ...
    public String upgrade(PartInfoVO player, RoleEntity roleEntity) {
        ...
        // 发送新手引导
        SystemInfoService.setNewPlayerGuideInfoMSG(roleEntity, 10 + "", player.getPGrade() + "");
        // 新浪的新手
        SystemInfoService.setNewPlayerGuideInfoMSG(roleEntity, 11 + "", player.getPGrade() + "");

        // 得到成长信息
        UGrowVO grow = getGrowInfo(roleEntity);
        upgrade_display = "恭喜您,您升到" + roleEntity.getBasicInfo().getGrade() + "级!气血增加" + grow.getGHP() + ",内力增加" + grow.getGMP() + ",攻击增加" + grow.getGGj() * SystemConfig.attackParameter + ",防御增加" + grow.getGFy() * SystemConfig.attackParameter + "!";
        return upgrade_display;
    }
}
