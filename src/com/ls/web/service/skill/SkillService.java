...
public class SkillService {
    ...
    public int skillAttackNPC(Fighter player, List npcs) {
        ...
        // 如果不可用则不做处理，返回
        if (!isUse) return skillInjure;
        /** *******************判断技能是否能使用结束*********************** */
        NpcService npcService = new NpcService();

        NpcFighter npc = (NpcFighter) npcs.get(0);
        int wxInjure = AttackService.getWxInjureValue(player.getWx(), npc, player.getPGrade(), npc.getLevel(), player.getPPk());
    ...
    public String studySkill(int p_pk, int sk_id) {
        ...
        if (playerSkillVO.getSkType() == 2 || playerSkillVO.getSkType() == 3 || playerSkillVO.getSkType() == 4) {
    ...
    public PlayerSkillVO getSkillOfPlayer(int p_pk, int sk_id) {
        // 得到角色skill信息
        PlayerSkillDao playerSkillDao = new PlayerSkillDao();
        PlayerSkillVO playerSkill = playerSkillDao.getPlayerSkillInfo(p_pk, sk_id);
    ...
    public PlayerSkillVO getSkillOfPlayerByGroup(int p_pk, int sk_Group) {
        // 得到角色skill信息
        PlayerSkillDao playerSkillDao = new PlayerSkillDao();
        PlayerSkillVO playerSkill = playerSkillDao.getSkillInfoByGroup(p_pk, sk_Group);
        loadSkillDetail(playerSkill);
        return playerSkill;
    }
    ...
    public void loadPassiveSkillEffectByDB(PartInfoVO player) {
        ...
        if (passSkillInterface != null) {
            ...
            // 增加玩家的 攻击力
            player.setPGj((int) (player.getPGj() + infovo.getPGj() * (passSkillInterface.getSkGjMultiple())));
            player.setPGj(player.getPGj() + passSkillInterface.getSkGjAdd());

            // 增加玩家的防御力
            player.setPFy((int) (player.getPFy() + infovo.getPFy() * (passSkillInterface.getSkFyMultiple())));
            player.setPFy(player.getPFy() + passSkillInterface.getSkFyAdd());

            // 增加玩家的mp
            player.setPUpMp((int) (player.getPUpMp() + infovo.getPUpMp() * (passSkillInterface.getSkMpMultiple())));
            player.setPUpMp(player.getPUpMp() + passSkillInterface.getSkMpAdd());

            // 增加玩家的hp
            player.setPUpHp((int) (player.getPUpHp() + infovo.getPUpHp() * (passSkillInterface.getSkHpMultiple())));
    ...
    private int studySkillTermsBySleight(int p_pk, String terms) {
        ...
        PlayerSkillVO vo = getSkillOfPlayerByGroup(p_pk, Integer.parseInt(terms_str[0].trim()));
        if (vo == null) {
            return -2;
        } else {
    ...
    private String updateSkill(int p_pk, int sk_id, PlayerSkillVO playerSkillVO) {
        ...
        // 将学习的技能放入内存中
        PlayerSkillVO playerSkillVO_new = playerSkillDao.getById(playerSkillVO.getSPk());
        ...
        roleShortCutInfo.updateShortcutName(playerSkillVO.getSPk(), skillvo.getSkName());
        // 修改该字符串 通知刘威
     ...
     */
    public String studySkillAll(int p_pk, PropVO prop, PropUseEffect propUseEffect) {
        String display = "";
        ...
        if (retult_one > 0) {
            ...
            return display;
        } else if (retult_one == -2) {
            display = "您已经学习过" + StringUtil.isoToGBK(getSkillName(study_sk_id));
            return display;
        }
        int retult_two = studySkillTermsBySleight(p_pk, terms_two);
        // 熟练度没达到要求
        if (retult_two == 0) {
            return "您的" + StringUtil.isoToGBK(getSkillName(study_sk_id)) + "的熟练度没达到要求!";
        }
        // 初始技能
        else if (retult_two == -2) {
            display = studySkill(p_pk, study_sk_id);
            return display;
        }
        // 技能升级的处理
        PlayerSkillVO vo = getSkillOfPlayerByGroup(p_pk, Integer.parseInt(terms_str_two[0].trim()));
