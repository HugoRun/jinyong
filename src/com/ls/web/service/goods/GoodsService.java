...
public class GoodsService {
    ...
    public int removeProps(PlayerPropGroupVO propGroup, int remove_num) {
        ...
        } else if (propGroup.getPropNum() > remove_num)// //移除的数量大于道具组数量
        {
            propGroupDao.updatePropGroupNum(propGroup.getPgPk(), propGroup.getPropNum() - remove_num);
            // 执行统计
            GameSystemStatisticsService gsss = new GameSystemStatisticsService();
            gsss.addPropNum(propGroup.getPropId(), StatisticsType.PROP, remove_num, StatisticsType.USED, StatisticsType.SHIYONG, 0);

        }
        if (propGroup.getPropType() == PropType.BOX_CURE || propGroup.getPropType() == PropType.FIX_ARM_PROP) { // 如果该道具是捆装药品,则必须将其也从特殊道具表中也删除
            propGroupDao.deletePropBoxCure(propGroup.getPgPk(), propGroup.getPPk());
        }
        return remove_num;
    }
    ...
    public int removeProps(int p_pk, int prop_id, int remove_num, int remove_type) {
        ...
        int current_groups = propGroupDao.getPropGroupNumByPropID(p_pk, prop_id);// 现有的组数
        ...
        logger.info("current_groups=" + current_groups + " ,new_groups=" + new_groups + " ,total_num=" + total_num);
        if (release_groups > 0) {
            // 释放玩家包裹need_groups空间
        ...
        {
            propGroupDao.updatePropGroupNumByPropID(p_pk, prop_id, goodsgourp_goodsnum);
        }
    ...
    public QueryPage getPageInlayStoneList(int p_pk, int page_no) {
        String prop_type_str = "0" +
                "," + PropType.EQUIP_INLAY_STONE_JIN +
                "," + PropType.EQUIP_INLAY_STONE_MU +
                "," + PropType.EQUIP_INLAY_STONE_SHUI +
                "," + PropType.EQUIP_INLAY_STONE_HUO +
                "," + PropType.EQUIP_INLAY_STONE_TU;

        PlayerPropGroupDao PlayerPropGroupDao = new PlayerPropGroupDao();
        return PlayerPropGroupDao.getPagePropByTypes(p_pk, prop_type_str, page_no);
    }
     ...
     * @param goods_id
     * @param goods_type
     * @param sendInfo   如果为一，就根据掉落物品是否是重要物品发送系统消息，如果为零，则不发。
     */
    public int pickupGoods(int p_pk, int drop_pk, int sendInfo, HttpServletResponse response, HttpServletRequest request) {
        ...
        result = putGoodsToWrap(p_pk, dropGoods.getGoodsId(), dropGoods.getGoodsType(), dropGoods.getGoodsQuality(), dropGoods.getDropNum(), GameLogManager.G_NPD_DROP);
        ...
        if (result != -1) {
            ...
            // 执行统计
            GameSystemStatisticsService gsss = new GameSystemStatisticsService();
            gsss.addPropNum(dropGoods.getGoodsId(), dropGoods.getGoodsType(), dropGoods.getDropNum(), StatisticsType.DEDAO, StatisticsType.DAGUAI, p_pk);
        }
        if (dropGoods.getDropGoodsInfo() != null && dropGoods.getGoodsType() != GoodsType.PROP) {
            if (sendInfo != 0) {
     ...
     * @param goods_type 放入物品的类型（EQUIP，PROP）
    ...
    public String putPropToWrap(String roleName, String propName, String propNum) {
        if (!StringUtil.isNumber(propNum)) {
            return "数量错误";
        }
    ...
    public int putPropToWrap(int p_pk, int goods_id, int goods_num, int gain_type) {
        ...
        if (current_num != 0) {
            // 因为玩家的包裹该物品格子不一定都是理想状态, 所以可能有零散的,故需要从数据库中取现有组数 .
            current_groups = propGroupDao.getPropGroupNumByPropID(p_pk, goods_id);
        }
        ...
        {
            propGroupDao.updateOneGroupPropNum(p_pk, prop_id, goodsgourp_goodsnum);
        }
        ...
        // 判断是否有这个道具
        PlayerPropGroupDao playerPropGroupDao = new PlayerPropGroupDao();
        String key_prop_id = GameConfig.getPropertiesObject("attainprop_mallprop");
        int pg_pk = playerPropGroupDao.getPgpkByProp(p_pk, Integer.parseInt(key_prop_id));
        if (pg_pk == -1) {
            int digit = GameConfig.getAttainProp(prop_id, "attain_prop_id");// 返回玩家等级是否在系统设定之内
            if (digit != -1) {
                new PopUpMsgService().addSysSpecialMsg(p_pk, prop_id, digit, PopUpMsgType.ATTAIN_PROP_TYPE);
            }
        }
    ...
    public int isPropToWrap(int p_pk, int goods_id, int goods_num) {
        ...
        int current_num = propGroupDao.getPropNumByByPropID(p_pk, goods_id);// 现有的数量
        int current_groups_sql = propGroupDao.getPropGroupNumByPropID(p_pk, goods_id); // 当前数据库中的组数
        ...
        if (wrap_spare == 0) {
            ...
            } else if (current_num + goods_num > current_groups_sql * accumulate_num) {
            ...
            if (current_num == 0) {
                ...
                if ((current_num + goods_num) > (current_groups_sql + wrap_spare) * accumulate_num) {
                    return -1;
                }
            }
        }
    ...
    public String putEquipToWrap(String roleName, String equipName, String equipNum, String qulity) {
        if (!StringUtil.isNumber(equipNum) || Integer.parseInt(equipNum) < 0) {
            return "数量错误";
        }
        if (!StringUtil.isNumber(qulity)) {
            return "品质错误";
        }
...
*/
     ...
     * 判断物品是否绑定
     *
     * @param goods_id   角色装备或道具的表id
     * @param goods_type 表示是装备还是道具
     * @param action     表示玩家的行为，如丢弃，买卖，交易，装备
     * @return
     */
     ...
     * @param goods_id
     * @param goods_type
     * @param action     表示玩家的行为，如丢弃，买卖，交易，装备，使用
     * @return
     */
    public String isBinded(GoodsControlVO goodsControl, int action) {
        ...
        if (goodsControl.getProtect() == 1)// 是否是受保护的
        {
            if (action == ActionType.SALE || action == ActionType.EXCHANGE || action == ActionType.THROW || action == ActionType.AUCTION) {
        ...
        } else if (goodsControl.getBonding() == BondingType.EXCHANGEBOND)// 交易绑定
        {
    ...
    public String saleProps(int pg_pk, int prop_num) {
        ...
        // 监控
        LogService logService = new LogService();
        logService.recordMoneyLog(roleInfo.getBasicInfo().getPPk(), roleInfo.getBasicInfo().getName(), roleInfo.getBasicInfo().getCopper() + "", price * prop_num + "", "卖道具ID" + propGroup.getPropId() + "数量" + prop_num);

        // 给用户增加钱数
        roleInfo.getBasicInfo().addCopper((long) price * prop_num);

        if (propGroup.getPgType() == 41) {
            SpecialPropDAO specialdao = new SpecialPropDAO();
            SpecialPropVO vo = specialdao.getEquipPropByPgpk(String.valueOf(pg_pk));
            if (vo != null) {
                specialdao.delEquipItem(String.valueOf(pg_pk));
            }
        }
        resultWml.append("您卖出" + StringUtil.isoToGBK(propGroup.getPropName()) + ",获得" + MoneyUtil.changeCopperToStr(price * prop_num));

        // 执行统计
        GameSystemStatisticsService gsss = new GameSystemStatisticsService();
        gsss.addPropNum(6, StatisticsType.MONEY, price, StatisticsType.DEDAO, StatisticsType.SELL, propGroup.getPPk());
    ...
    public String getPropInfoWmlMai(int pPk, int prop_id) {
        ...
        if (prop.getPropJob() != null && !prop.getPropJob().equals("") && !prop.getPropJob().equals("0")) {
    ...
    public String getPropInfoWml(int pPk, int prop_id) {
         ...
         * 使用 丢弃
         */
        String resultWml = prop.getPropName() +
                "<br/>" +
                propPic +
                prop.getPropDisplay() +
                "<br/>" +
                "--------------------" +
                "<br/>" +
                "卖出价格:" + prop.getPropSell() +
                "灵石" +
                "<br/>" +
                "使用级别:" + getPropDisplay(prop.getPropReLevel()) + "级" +
                "<br/>" +
                "性别要求:" + ExchangeUtil.exchangeToSex(prop.getPropSex()) +
                "<br/>" +
                "职业要求:" + titleService.getTitleNamesByTitleIDs(prop.getPropJob()) +
                "<br/>";
        return resultWml;
    }
    ...
    public String getPropInfoWml(int pPk, int prop_id, String pg_pk) {
        ...
        resultWml.append("<br/>");
        if (prop.getPropClass() != PropType.BOX_CURE) { // 如果道具是捆装类药品, 就让其显示出剩余血量或内力量
            resultWml.append("------------------------");
            resultWml.append("<br/>");
        } else {
            resultWml.append("剩余可治疗量").append(getSpecialPropNum(pPk, pg_pk, prop.getPropOperate2(), SpecialNumber.KUNZHUANG)).append("点").append("<br/>");
        }
     ...
     * @return
     */
    public String getPropInfoWml(int pPk, int prop_id, String w_type, String pg_pk, String goods_type, String isReconfirm, HttpServletRequest request, HttpServletResponse response) {
        RoleEntity roleInfo = RoleService.getRoleInfoById(pPk + "");
     ...
     * @return
     */
    private int getSpecialPropNum(int pk, String pg_pk, String propOperate2, int sp_type) {
    ...
    public String buyGoods(RoleEntity roleInfo, int npcshop_id, int goods_num) {
        ...
        if (money < total_price) {
            resultWml.append("购买" + goods_num + "个" + npcshop.getGoodsName() + ",需要" + MoneyUtil.changeCopperToStr(total_price) + ",您的钱数不够!<br/>");
        } else {
            // 购买
            if (putGoodsToWrap(roleInfo.getPPk(), npcshop.getGoodsId(), npcshop.getGoodsType(), goods_num) == -1) {
                ...
                // 监控
                LogService logService = new LogService();
                logService.recordMoneyLog(roleInfo.getPPk(), roleInfo.getName(), roleInfo.getBasicInfo().getCopper() + "", -total_price + "", "买道具ID" + npcshop.getGoodsName() + "数量" + goods_num);
    ...
    public void dropGoods(Fighter loser, Fighter winer) {
        ...
        if (drop_num > 0) {
            ...
            if (drop_list.getItemGroupNum() > 0) {
                ...
                mail_info.createPKDropItemMail(winer.getPPk(), //邮件标题
                        loser.getPName() + "被你杀死掉落的物品", drop_list.getDes(), drop_list);
    ...
    private UEquip dropEquip(Fighter loser, List<Integer> equip_key_list) {
        ...
        if (equip_key_list != null && equip_key_list.size() != 0) {
            ...
            if (!equipvo.isProtectOnPK())//不受保护的装备
            {
                if (equipvo.isOnBody()) {
     ...
     * @param add_mp
     */
    public void reduceBoxCure(PlayerPropGroupVO propGroup, int add_mp, int sp_type) {
        PlayerPropGroupDao propGroupDao = new PlayerPropGroupDao();
    ...
    public int pickupAllGoods(int p_pk) {
        ...
        logger.info("equipGoods.size=" + equipGoods.size() + " ,dropGoods.size=" + dropGoods.size());
        ...
        for (int i = dropGoods.size() - 1; i >= 0; i--) {
            ...
            logger.info("name=" + dropGoodsVO.getGoodsName() + " ,eachPropSpare[0]=" + eachPropSpare[0] + " ,eachPropSpare[1]=" + eachPropSpare[1]);
        ...
        if (handleWrapSpare >= 0) {
            ...
            for (int i = equipGoods.size() - 1; i >= 0; i--) {
                ...
                putGoodsToWrap(p_pk, dropGoodsVO.getGoodsId(), dropGoodsVO.getGoodsType(), dropGoodsVO.getGoodsQuality(), dropGoodsVO.getDropNum(), GameLogManager.G_NPD_DROP);
                // 执行统计
                GameSystemStatisticsService gsss = new GameSystemStatisticsService();
                gsss.addPropNum(dropGoodsVO.getGoodsId(), dropGoodsVO.getGoodsType(), dropGoodsVO.getDropNum(), StatisticsType.DEDAO, StatisticsType.DAGUAI, p_pk);
            }
     ...
     *
     * @param dropGoodsVO
     * @param need_group  第一个为道具的占据包裹空间大小, 第二个为插入时剩余道具组的道具数量
     */
    public void putInWareSpare(DropGoodsVO dropGoodsVO, Integer[] need_group) {
        ...
        propGroup = propGroupDao.getPropGroupByPropID(p_pk, dropGoodsVO.getGoodsId());
        if (goodsgourp_goodsnum != 0) {
            propGroupDao.updatePropGroupNum(propGroup.getPgPk(), goodsgourp_goodsnum);
        }
        if (need_groups == 0 && goodsgourp_goodsnum == 0 && goods_num != 0) // 不需要加道具组，正好装满道具组
        {
            propGroupDao.updatePropGroupNum(propGroup.getPgPk(), accumulate_num);
        }
     ...
     * @return
     */
    public int returnpropspare(RoleEntity roleInfo, int p_pk, int goods_id, int goods_num) {
    ...
    public boolean isEnoughWrapSpace(int pPk, int number) {
        ...
        return (wrap_spare - number) >= 0;
    }

    /**
     * 装备类道具的描述
     */
    public String getEquipPropInfoWml(int pPk, int prop_id, String w_type, String pg_pk, String goods_type, String isReconfirm, String wupinlan, HttpServletRequest request, HttpServletResponse response) {
        RoleEntity roleInfo = RoleService.getRoleInfoById(pPk + "");
         ...
         */
        if (prop.getPropClass() == PropType.EQUIPPROP || prop.getPropClass() == PropType.JIEHUN_JIEZHI) {
            resultWml.append(prop.getPropName());
            resultWml.append("<br/>");
            resultWml.append(propPic);
            resultWml.append(prop.getPropDisplay());
            ...
            resultWml.append("------------------------");
            resultWml.append("<br/>");
            resultWml.append("卖出价格:" + MoneyUtil.changeCopperToStr(prop.getPropSell()));
            resultWml.append("<br/>");
            resultWml.append("使用级别:" + getPropDisplay(prop.getPropReLevel()) + "级");
            resultWml.append("<br/>");
            resultWml.append("性别要求:" + ExchangeUtil.exchangeToSex(prop.getPropSex()));
            resultWml.append("<br/>");
            resultWml.append("职业要求:" + titleService.getTitleNamesByTitleIDs(prop.getPropJob()));
            resultWml.append("<br/>");
            if (prop.getPropClass() == PropType.JIEHUN_JIEZHI) {
                resultWml.append("爱情甜蜜:" + new FriendService().findLove_dear(pPk));
                resultWml.append("<br/>");
            }
            ...
            if (prop.getPropClass() == PropType.EQUIPPROP) {
                ...
                resultWml.append("剩余时间:" + time + "分钟/" + prop.getPropOperate2() + "分钟");
                resultWml.append("<br/>");
            }
        }
        if (prop.getPropClass() == PropType.EQUIPPROP) {
            if (vo == null) {
                ...
                // href=\"/stateaction.do\"> ");
                resultWml.append("<go method=\"post\" href=\"" + response.encodeURL(GameConfig.getContextPath() + "/stateaction.do") + "\">");
                ...
                resultWml.append("<postfield name=\"wupinlan\" value=\"" + wupinlan + "\" /> ");
                resultWml.append("<postfield name=\"goods_type\" value=\"2\" /> ");
                resultWml.append("</go>");
                resultWml.append("使用 ");
                resultWml.append("</anchor> |");
                ...
                // resultWml.append("<go method=\"post\" sendreferer=\"true\"
                // href=\"/wrap.do\"> ");
                resultWml.append("<go method=\"post\" href=\"" + response.encodeURL(GameConfig.getContextPath() + "/wrap.do") + "\">");
                resultWml.append("<postfield name=\"cmd\" value=\"n7\" /> ");
                resultWml.append("<postfield name=\"w_type\" value=\"" + w_type + "\" /> ");
                resultWml.append("<postfield name=\"pg_pk\" value=\"" + pg_pk + "\" /> ");
                resultWml.append("<postfield name=\"goods_id\" value=\"" + prop_id + "\" /> ");
                resultWml.append("<postfield name=\"goods_type\" value=\"" + goods_type + "\" /> ");
                resultWml.append("<postfield name=\"isReconfirm\" value=\"" + isReconfirm + "\" /> ");
                resultWml.append("</go>");
                resultWml.append("丢弃");
                resultWml.append("</anchor> ");
                resultWml.append("<br/>");
                ...
                if (vo.getProptime() == 0) {
                    ...
                    // sendreferer=\"true\" href=\"/stateaction.do\"> ");
                    resultWml.append("<go method=\"post\" href=\"" + response.encodeURL(GameConfig.getContextPath() + "/stateaction.do") + "\">");
                    resultWml.append("<postfield name=\"cmd\" value=\"n21\" /> ");
                    resultWml.append("<postfield name=\"pg_pk\" value=\"" + pgpk + "\" /> ");
                    resultWml.append("<postfield name=\"w_type\" value=\"" + w_type + "\" /> ");
                    resultWml.append("<postfield name=\"goods_type\" value=\"2\" /> ");
                    resultWml.append("<postfield name=\"wupinlan\" value=\"1\" /> ");
                    resultWml.append("</go>");
                    resultWml.append("更换 ");
                    resultWml.append("</anchor> ");
                }
...
*/
    ...
    public int updateMiJingOwner(int last_owner, int new_owner) {
        ...
        int result = new PlayerPropGroupDao().updateMiJingOwner(last_owner, new_owner);
        if (result > 0) {
            EquipService equipService = new EquipService();
