...
public class EquipService {
        ...
        if (list != null && list.size() != 0) {
            for (int i = 0; i < list.size(); i++) {
                ...
                if (wDuraConsume == 209) {
                    ...
                    String hintss = StringUtil.isoToGB(vo.getWName()) + "的耐久小于20，尽快修复装备！";
                    ms.sendMailBySystem(pPk, title, hintss);
                }
                if (wDuraConsume == 59) {
                    MailInfoService ms = new MailInfoService();
                    String title = "装备耐久小于5";
                    String hintss = StringUtil.isoToGB(vo.getWName()) + "的耐久小于5，尽快修复装备！";
                    ms.sendMailBySystem(pPk, title, hintss);
                }
                if (wDuraConsume <= 0) {
                    hint = "装备持久已经为0不能在相减";
    ...
    public String useWeapon(int pPk) {
        ...
        if (weapon != null) {
            ...
            if (wDuraConsume == 209) {
                MailInfoService ms = new MailInfoService();
                String title = "装备耐久小于20";
                String hintss = StringUtil.isoToGB(weapon.getWName()) + "的耐久小于20，尽快修复装备！";
                ms.sendMailBySystem(pPk, title, hintss);
            }
            if (wDuraConsume == 59) {
                MailInfoService ms = new MailInfoService();
                String title = "装备耐久小于5";
                String hintss = StringUtil.isoToGB(weapon.getWName()) + "的耐久小于5，尽快修复装备！";
                ms.sendMailBySystem(pPk, title, hintss);
            }
            if (wDuraConsume <= 0) {
                hint = "装备持久已经为0不能在相减";
    ...
    public String relelaEquip(RoleEntity roleEntity, PlayerEquipVO equipVO, HttpServletResponse response, HttpServletRequest request) {
        ...
        String sBuffer = "玩家" + roleEntity.getName() + "展示了他的装备" +
                "<anchor>" +
                "<go method=\"post\" href=\"" + response.encodeURL(GameConfig.getContextPath() + "/equiprelela.do") + "\">" +
                "<postfield name=\"cmd\" value=\"n2\" /> " +
                "<postfield name=\"pwpk\" value=\"" + equipVO.getPwPk() + "\" /> " +
                "</go>" +
                equipVO.getWName() +
                "</anchor> ";
        ...
        systemInfoService.insertSystemInfoByBackTong(sBuffer, time, SystemInfoType.EQUIPRELELA);

        // 花费50个元宝
        EconomyService economyService = new EconomyService();
        economyService.spendYuanbao(roleEntity.getBasicInfo().getUPk(), 50);

        return "你已经花费" + GameConfig.getYuanbaoName() + "×50展示了你的装备" + equipVO.getWName();
