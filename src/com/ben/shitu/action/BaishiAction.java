...
public class BaishiAction extends BaseAction {
    ...
    public ActionForward n1(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (bi.getGrade() >= ShituConstant.BAISHI_LEVEL_LIMIT) {
            setMessage(request, "对不起，只有" + ShituConstant.BAISHI_LEVEL_LIMIT + "级以下的玩家才可拜师！");
        ...
        if (count != 0) {
            setMessage(request, "对不起，你已经有了一位师傅！如想另投名师可在游戏“系统”栏的“师徒管理”中解除师徒关系后再次拜师！");
            return mapping.findForward(ERROR);
        }
        int count1 = shituService.findCount(ShituConstant.BAISHISQL2, args);
        if (count1 == 0) {
            Shitu shitu = new Shitu(0, 0, bi.getPPk(), null, bi.getName(), 0, bi.getGrade(), null, null);
    ...
    public ActionForward n4(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (fv != null && fv.getRelation() != 0) {
            setMessage(request, "对不起，您和对方是" + (fv.getRelation() == 1 ? "结义" : "结婚") + "关系，不能拜对方为师");
            return mapping.findForward(ERROR);
        }
        String message = bi.getName() + "想拜你为师.<br/>" + "<anchor><go method=\"post\" href=\"" + response.encodeURL(GameConfig.getContextPath() + "/baishi.do") + "\">" + "<postfield name=\"cmd\" value=\"n5\" />" + "<postfield name=\"id\" value=\"" + id + "\" />" + " <postfield name=\"stu_id\" value=\"" + bi.getPPk() + "\" />" + "<postfield name=\"caozuo\" value=\"0\" />" + " </go>" + "同意</anchor><br/>" + "<anchor><go method=\"post\" href=\"" + response.encodeURL(GameConfig.getContextPath() + "/baishi.do") + "\">" + "<postfield name=\"cmd\" value=\"n5\" />" + "<postfield name=\"id\" value=\"" + id + "\" />" + "<postfield name=\"stu_id\" value=\"" + bi.getPPk() + "\" />" + "<postfield name=\"caozuo\" value=\"1\" />" + " </go>" + "拒绝</anchor><br/>";
        mailInfoService.sendMailBySystem(vo.getPPk(), bi.getName() + "想拜你为师", message);
    ...
    public ActionForward n5(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (stu_id == null || "".equals(stu_id.trim()) || id == null || "".equals(id.trim()) || caozuo == null || "".equals(caozuo.trim())) {
            setMessage(request, "出错了，请重新操作");
            return mapping.findForward(ERROR);
        }
        BasicInfo bi = getBasicInfo(request);
        ...
        if (Integer.parseInt(caozuo.trim()) == 1) {
            ...
            mailInfoService.sendMailBySystem(vo.getPPk(), bi.getName() + "拒绝收您为徒", bi.getName() + "拒绝收您为徒");
            return mapping.findForward(ERROR);
        }
        ...
        String message = "恭喜你，你已经是玩家" + bi.getName() + "的弟子了，你可以获得以下好处：<br/>" + "1.	可接受师傅每天直接传功一次<br/>" + "2.	师徒同时在线，可额外获得打怪经验的20%的经验<br/>" + "3.	完成各类任务时可获得1.5倍的任务奖励<br/>" + "4.	可在系统栏的“师徒管理”内查看师徒各项关系<br/>";

        String message1 = vo.getPName() + "拜你为师，以后一心一意的伺候你。";
        setMessage(request, message1);
        mailInfoService.sendMailBySystem(vo.getPPk(), bi.getName() + "同意收您为徒", message);
        // systemInfoService.insertSystemInfoBySystem(vo.getPPk(),message1);

        // 从拜师列表中删除自己
        shituService.doit(ShituConstant.BAISHISQL4, new Object[]{vo.getPPk()});
        shituService.remove(vo.getPName(), bi.getName());
        return mapping.findForward(ERROR);
    }

    public ActionForward nn1(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        BasicInfo bi = getBasicInfo(request);
    ...
    public ActionForward nn2(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        BasicInfo bi = getBasicInfo(request);
        String mailId = request.getParameter("mailId");
        if (mailId == null || "".equals(mailId.trim())) {
            setMessage(request, "出错了，请重新操作");
