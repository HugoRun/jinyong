...
public class TiaoZhanAction extends BaseAction
{
			...
			HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (pg_pk != null && !"".equals(pg_pk.trim()))
		{
			...
			if (propGroup != null)
			{
				...
				systemInfoService.insertSystemInfoBySystem(bi.getRealName() + "向"
						+ name.trim() + "下了战书！不知" + name.trim() + "会做何应对！");
	...
	private String getSystemMessage(String name1,String name2){
		...
		switch(numm){
			case 0:break;
			case 1: mess = name1+"被"+name2+"的气势所压倒,居然悄悄的溜走了,留下的只有"+name2+"鄙视的目光。";break;
			case 2: mess = name2+"把胸脯拍得噼啪响，对"+name1+"咆哮道：“飞毛腿了不起啊,逃的还真快,有种的你过来啊!”";break;
			case 3: mess = name2+"对"+name1+"逃去的身影咆哮道:\"胆小鬼,不要跑,我们大战三百回合\"。";break;
			case 4: mess = name2+"对"+name1+"摆开架式准备对战,"+name1+"突然消失了,"+name2+"感叹到\""+name1+"你怕死就不要来嘛,浪费时间\"。";break;
			default : break;
		}
		return mess;
	}
			...
			HttpServletRequest request, HttpServletResponse response)
	{
		...
		else
		{
			...
			// 进入擂台
			systemInfoService.insertSystemInfoBySystem(bi.getRealName() + "接受了"
					+ tiao_name + "的挑战！双方站到了生死擂台上！");
