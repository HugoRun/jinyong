...
public class LotteryAction extends DispatchAction {
    ...
    public ActionForward n2(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        LotteryService se = new LotteryService();
        Integer lotterytype = Integer.parseInt(request.getParameter("lotterytype"));
        if (!se.sysTimeAtEight()) {
            request.setAttribute("lotterydisplay", "投注时间为每天的8:00～20:00，您不前不能投注!");
            return mapping.findForward("lottery_display");
        } else {
            if (lotterytype == 0) {
                if (se.playerGuessLottery(roleInfo.getBasicInfo().getPPk(), lotterytype)) {
                    request.setAttribute("lotterydisplay", "对不起,您已经参加过本期彩票竞猜,每人每天只能竞猜一次,请耐心等待开奖结果!");
                    return mapping.findForward("lottery_display");
                } else {
                ...
                if (se.playerGuessLottery(roleInfo.getBasicInfo().getPPk(), lotterytype)) {
                    ...
                    if (!se.getPlayerCharityLottery(roleInfo.getBasicInfo().getPPk())) {
                        request.setAttribute("lotterydisplay", "对不起,您没有资格参加慈善竞猜!<br/>每月投注总数超过15次,中奖0次才有资格参加慈善竞猜!");
                        return mapping.findForward("lottery_display");
                    }
    ...
    public ActionForward n3(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (money == 1) {
            ...
            request.setAttribute("lotterydisplay", "请正确投注!");
            return mapping.findForward("lottery_display");
        }
    }
    ...
    public ActionForward n4(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (now.getHours() == 20) {
            ...
            if (lotterytype == 0) {
                ...
                se.playerLotteryNumber(pPk, number1, number2, number3, number4, lotterytype, M);

                request.setAttribute("lotterydisplay", "您本期投注:" + number1 + "," + number2 + "," + number3 + "," + number4 + "," + "价值" + M / 100 + "两!<br/>本期竞猜20:00开奖,21:00～24:00领取奖励,过期后您的奖励将自动进入下期奖励!");
                dao.updateBotteryBonus(M);
                return mapping.findForward("lottery_display");
            } else {
                se.playerLotteryNumber(pPk, number1, number2, number3, number4, lotterytype, money);

                request.setAttribute("lotterydisplay", "您本期慈善投注:" + number1 + "," + number2 + "," + number3 + "," + number4 + "!<br/>本期竞猜20:00开奖,21:00～24:00领取奖励,过期后您的奖励将自动进入下期奖励!");
                return mapping.findForward("lottery_display");
            }
        }
    }
                ...
                if (se.ifPlayerCatch(pPk) == 2) {
                    ...
                    if (vo.getLotteryCatchMoney() == 1) {
                        ...
                        if (se.getPlayerWrapSpare(pPk) == -1) {
                            request.setAttribute("lotterydisplay", "您的包裹已满请清理包裹后再来!");
                            return mapping.findForward("lottery_display");
                        } else {
                            request.setAttribute("lotterydisplay", "您获得" + MoneyUtil.changeCopperToStr(se.playerHaveMoney(pPk)) + "!");
        ...
        if (!se.sysTimeAtTwenty()) {
            ...
            if (se.ifPlayerCatchCharity(pPk) == 3) {
                ...
                if (se.ifPlayerCatchCharity(pPk) == 2) {
                    ...
                    request.setAttribute("lotterydisplay", "您获得" + MoneyUtil.changeCopperToStr(se.playerHaveCharityMoney(pPk)) + "!");
